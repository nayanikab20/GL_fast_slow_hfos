...
function [zmap]=stats_high_gran(freq1,freq2, freqrange, a)
%ntrials=size(freq1.powspctrm,1);
ntrials=size(freq1.grangerspctrm, 5);

%Requires converting NaNs values into zeros.
no1=squeeze(freq1.grangerspctrm(a(1),a(2),freqrange(1) :freqrange(2),:, :));
no2=squeeze(freq2.grangerspctrm(a(1), a(2),freqrange(1) :freqrange(2),:, :));
...
no1=reshape(no1, [freqrange(2), 201, ntrials]);
no2=reshape(no2, [freqrange(2), 201, ntrials]);
%%
%freq1.powspctrm=no1;
%freq2.powspctrm=no2;
...
permmaps = zeros(n_permutes,freqrange(2),length(freq1.time));
